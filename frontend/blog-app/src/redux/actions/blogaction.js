...
import {
  ...
  GET_LIKES_SUCCESS,
  GET_MORE_BLOGS_FAIL,
  GET_MORE_BLOGS_REQUEST,
  GET_MORE_BLOGS_SUCCESS,
  INCREASE_VIEWS_COUNT_FAIL,
  INCREASE_VIEWS_COUNT_SUCCESS,
  LIKE_FAIL,
  LIKE_SUCCESS,
  NEW_BLOG_FAIL,
  NEW_BLOG_REQUEST,
...
export const loadBlogs = () => async (dispatch) => {
  try {
    ...
    const data = await axios(`${process.env.REACT_APP_BACKEND_URL}blog/`, {
...
//! ADD COMMENT
export const addCommentAPI = ({ id, content }) => async (dispatch) => {
  try {
...
export const updateBlog = (blogData, navigate) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_BACKEND_URL}blog/${blogData.id}/`,
      blogData,
      {
        headers: {
          Authorization: `Token ${token}`,

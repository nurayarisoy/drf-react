...
const PostDetails = () => {
  ...
  const { blog, loading, error, viewSuccess } = useSelector(
  ...
  const { isLiked, error: likeError, likes } = useSelector(
    (state) => state.like,
  )
            ...
            subheader={
              ...
              >
                ...
                <Typography variant="body2">
                  {blog?.days_since_creation > 0
                    ? blog?.days_since_creation + ' days ago'
                    : 'today'}
                </Typography>
              </div>
            }
          ...
          >
            <div>
              <IconButton aria-label="add to favorites" onClick={likeBlog}>
                {!blog?.likes.find(
                  (like) => like.user === (user?.id || user?.pk),
                ) ? (
                  <BiLike color="#c9c9c9" />
                ) : (
                ...
                &nbsp;
                <p style={{ fontSize: '1rem', color: '#c9c9c9' }}>
                  {blog?.like_count}
                </p>
              </IconButton>
            ...
            {(user?.pk || user?.id) === blog?.user && (
              <div>
                ...
                <IconButton
                  ...
                  size="large"
                  color="error"
                  onClick={() => dispatch(deleteBlogAPI(blog.id, navigate))}
                  aria-label="share"
                >
